...
class Restoran {
    ...
    public void generisiObracun() {
    	System.out.printf("%-5s %-10s %-10s %-12s %-15s %-10s\n", "ID", "Ime", "Prezime", "Tip", "Broj sati", "Plata");
        double ukupno = 0;
        for (Zaposleni z : zaposleni) {
            String dodatno = "";
            if (z instanceof Konobar) dodatno = " (prek: " + ((Konobar) z).getPrekovremeniSati() + ")";
            if (z instanceof Menadzer) dodatno = " (bonus: " + ((Menadzer) z).getBonus() + ")";
            double plata = z.mesecnaPlata();
            ukupno += plata;
            System.out.printf("%-5d %-10s %-10s %-12s %-15.2f %-10.2f%s\n", z.getId(), z.getIme(), z.getPrezime(), z.tipZaposlenog(), z.getUkupanBrojSati(), plata, dodatno);
        }
